...
/* Function */
  ...
  function _SearchID(db, id) {
    return new Promise((resolve, reject) => {
      let sql ="SELECT `ID`, `name`, `classification_id`, `location`, `registered_time`, `time_interval_LB`, `time_interval_UB`, `department_id`, `registrant_id`, `description`, `state`, `image` FROM `property_found` WHERE `ID` = ? order by time_interval_LB DESC";
  ...
  function _SearchAuthor(db, author) {
    return new Promise((resolve, reject) => {
      let sql = "SELECT `ID`, `name`, `classification_id`, `location`, `registered_time`, `time_interval_LB`, `time_interval_UB`, `department_id`, `registrant_id`, `description`, `state`, `image`, `department_id` FROM `property_found` WHERE `registrant_id` = ? order by time_interval_LB DESC";
...
/* Router */
  ...
  router.get('/author/:author', function(req, res, next) {
    ...
    _SearchAuthor(db, author).then(foundObj => {
      res.send(foundObj);
      return;
    }).catch(errorObj => {
      res.status(404).send(errorObj);
